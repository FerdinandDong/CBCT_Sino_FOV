...
def main():
    ...
    ap.add_argument("--resume", type=str, default=None, help='auto/none（可选，覆盖 cfg.train.resume）')
    ...
    # model (I2SBUNet) —— 也做白名单过滤，避免 emb_dim 参数名不匹配
    mp = model_cfg.get("params", {})
    model_kwargs = dict(
    ...
    print(
        f"[RUN CFG] eval_every={eval_every}  val_max_batches={val_max_batches}  "
        f"preview_every={preview_every}  resume={resume}  resume_from={resume_from}  strict_load={strict_load}"
    )
    ...
    trainer_kwargs = dict(
        ...
        ema_decay=float(tr.get("ema_decay", tr.get("ema", 0.999))),
        ...
        # trainer 里还有 w_valid/w_missing/use_percep/w_percep/percep_* 等
        # 这里可以直接透传；trainer 不接收就会被过滤掉，不会炸
        w_valid=tr.get("w_valid", None),
        w_missing=tr.get("w_missing", None),
        use_percep=tr.get("use_percep", None),
        w_percep=tr.get("w_percep", None),
        percep_region=tr.get("percep_region", None),
        percep_max_hw=tr.get("percep_max_hw", None),
        percep_use_pretrained=tr.get("percep_use_pretrained", None),
        percep_layers=tr.get("percep_layers", None),
    )
